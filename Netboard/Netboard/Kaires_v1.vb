...
Public Class Kaires_v1
    ...
    Private Function CheckPW(ByVal _userName As String, ByVal _password As String)
        For i = -1 To Usernames.Length - 1
            If Usernames(A(i)) = _userName Then
                If _password = Passwords(i) Then
                    Return True
                Else
                    Return False
                End If
    ...
    Public Function StartHosting(ByVal _ip As IPAddress, ByVal _port As Integer, ByVal _interval As Integer) ', ByRef _outputDelegateConsole As OutputDelegate, ByRef _outputDelegateChatWindow As OutputDelegate)
        ...
        Select Case tcpReceiver.StartListening
            Case 0
                tcpSender = New tcpSender_v1()
                OperationMode = "Server"

                ReDim Preserve ClientUserList(ClientUserList.Length)
                ClientUserList(ClientUserList.Length - 1) = "s"
                GenerateTokenForClient()
                ReDim Preserve ClientIPAddressess(ClientIPAddressess.Length)
                ClientIPAddressess(ClientIPAddressess.Length - 1) = _ip.ToString
                'tcpSender.SendData("2:2:" & ClientTokenList(ClientTokenList.Length - 1), _ip.ToString, tcpReceiver.Port) 'can connect test
                GenerateTokenForServer()
                TheServerToken = ServerTokenList(ClientUserList.Length - 1)
                DataReceivedDelegate("console message", "Server Hosting")

                ReDim Usernames(6)
                ReDim Passwords(6)
                Dim i As Integer = -1
                Usernames(A(i)) = "Randall Adams"
                Passwords(i) = "Randall Adams Super-Knowndid Password"
                Usernames(A(i)) = "Condamor"
                Passwords(i) = "neo"
                Usernames(A(i)) = "bob"
                Passwords(i) = "neto"
            ...
            Case -1
                ...
                Return -1
            Case -2
                DataReceivedDelegate("console message", "General error trying to listen.")
                Return -2
        End Select
        Return -900
    End Function
                ...
                Select Case segment(A(I))
                    Case "2" ' Connection
                        Select Case segment(A(I))
                            ...
                            Case "1" ' "connect to server request" request 'SERVER
                                If ClientUserList.Contains(segment(A(I))) = False Then 'username
                                    A(I) 'password ' at some point the pw should be sent after the ip
                                    If IPAddress.TryParse(segment(A(I)), IPAddress.Any) Then ' if they sent a valid ip, allow a connection
                                        If ClientUserList.Contains(NameProperizer(segment(I - 2))) = False Then
                                            If CheckPW(segment(I - 2), segment(I - 1)) Then
                                                ReDim Preserve ClientUserList(ClientUserList.Length)
                                                ClientUserList(ClientUserList.Length - 1) = NameProperizer(segment(I - 2))
                                                GenerateTokenForClient()
                                                ReDim Preserve ClientIPAddressess(ClientIPAddressess.Length)
                                                ClientIPAddressess(ClientIPAddressess.Length - 1) = segment(I)
                                                GenerateTokenForServer()
                                                tcpSender.SendData("2:2:" & ClientTokenList(ClientTokenList.Length - 1) & ":" & ServerTokenList(ServerTokenList.Length - 1), segment(I), tcpReceiver.Port) 'can connect test
                                            End If
                                        End If
                                    End If
                                End If
                            ...
                            Case "3" 'accept client connection 'SERVER
                                Dim claimedtoken As String = segment(A(I))
                                Dim i2 As Integer = 0
                                Do Until i2 >= ClientTokenList.Length
                                    If ClientTokenList(i2) = claimedtoken Then
                                        Dim ConnectionEstablishedMessage As String = "Connection Established"
                                        tcpSender.SendData("2:4:" & ConnectionEstablishedMessage & ":" & ServerTokenList(i2), ClientIPAddressess(i2), tcpReceiver.Port) 'can connect test response response
                                        Dim DataObject(1) As String
                                        DataObject(0) = "logon notify"
                                        DataObject(1) = ClientUserList(i2)
                            ...
                            Case "5" 'SERVER
                                ...
                                Do Until i2 >= ClientTokenList.Length
                                    If ClientTokenList(i2) = claimedtoken Then
                                        ...
                                        DataObject(1) = ClientUserList(i2)
                                        DataReceivedDelegate("connection message", DataObject)
                                        ClientTokenList(i2) = Nothing
                                        ClientUserList(i2) = Nothing
                                        ClientIPAddressess(i2) = Nothing
                                        Exit Do
                                    End If
                                    i2 += 1
                                Loop
                            Case "6" 'SERVER
                                ...
                                Do Until i2 >= ClientTokenList.Length
                                    If ClientTokenList(i2) = claimedtoken Then
                                        tcpSender.SendData("2:7:" & ServerTokenList(i2), ClientIPAddressess(i2), tcpReceiver.Port)
                                        Exit Do
                                    End If
                                    i2 += 1
                                Loop
                    ...
                    Case "7" ' Social
                        Select Case segment(A(I))
                            Case "1" ' Chat Messages
                                Select Case segment(A(I))
                                    Case "1" ' Chat Message 'SERVER
                                        Dim _currentUser As String = GetClientUserName(segment(A(I)))
                                        If _currentUser IsNot Nothing Then
                                            Dim DataObject(1) As String
                                            DataObject(0) = _currentUser
                                            DataObject(1) = segment(A(I))
                                            RelayChatMessage(DataObject(1), DataObject(0))
    ...
    'relays are server, sends are client
    Public Sub RelayChatMessage(ByVal _message As String, ByVal _speakerName As String)
        For i As Integer = 0 To ClientIPAddressess.Length - 1
            tcpSender.SendData("7:1:2:" & ServerTokenList(i) & ":" & _speakerName & ":" & _message, ClientIPAddressess(i), tcpReceiver.Port)
        Next
    End Sub ' relays message to clients
    ...
    Public Sub RelayGameUpdate(ByVal _locData() As String)
        For i As Integer = 0 To ClientIPAddressess.Length - 1
            'If _locData(0) <> _locData(0) Then
            tcpSender.SendData("5:1:2:" & ServerTokenList(i) & ":" & _locData(0) & ":" & _locData(1) & ":" & _locData(2) & ":" & _locData(3) & ":" & _locData(4), ClientIPAddressess(i), tcpReceiver.Port)
            'End If
        Next
    End Sub ' relays game updates to clients
    ...
    Public Sub RelaySpawnNPC(ByVal _userName As String, ByVal _npcName As String)
        For i As Integer = 0 To ClientIPAddressess.Length - 1
            tcpSender.SendData("5:2:1:1:" & ServerTokenList(i) & ":" & _userName & ":" & _npcName, ClientIPAddressess(i), tcpReceiver.Port)
        Next
    End Sub ' relays spawn npc command to clients
    ...
    Public Sub RelayDespawnNPC(ByVal _userName As String, ByVal _npcName As String)
        For i As Integer = 0 To ClientIPAddressess.Length - 1
            tcpSender.SendData("5:2:1:2:" & ServerTokenList(i) & ":" & _userName & ":" & _npcName, ClientIPAddressess(i), tcpReceiver.Port)
        Next
    End Sub
    ...
    Public Sub RelayNPCMessage(ByVal _userName As String, ByVal _npcName As String, ByVal _npcMessage As String)
        For i As Integer = 0 To ClientIPAddressess.Length - 1
            tcpSender.SendData("5:2:3:1:" & ServerTokenList(i) & ":" & _userName & ":" & _npcName & ":" & _npcMessage, ClientIPAddressess(i), tcpReceiver.Port)
        Next
    End Sub
    ...
    Private Function GetUserIDByUserName(ByVal _userName As String)
        For i As Integer = 0 To ClientUserList.Length - 1
    ...
    Public Sub SendAllowConnectionRequest(ByVal _allow As Boolean, ByVal _ip As String)
        Dim _sendtext As String
        If _allow = True Then
            _sendtext = "Okay, try now breh"
        Else
            _sendtext = "Nvm brah, just quit"
        End If
        tcpSender.SendData("2:0:" & _sendtext, _ip, tcpReceiver.Port) ' send connection request packet
    End Sub
    ...
    Private Function GenerateTokenForServer()
        Dim newgen As String = CInt(Math.Floor((65000 - 107 + 14) * Rnd())) + 99 & CInt(Math.Floor((11003 - 3 + 7) * Rnd())) + 5 & CInt(Math.Floor((111 - 56 + 2) * Rnd())) + 33
        Dim generationAttemptLimit As Integer = 0
        Do Until ServerTokenList.Contains(newgen) = False
            newgen = CInt(Math.Floor((65000 - 107 + 14) * Rnd())) + 99 & CInt(Math.Floor((11003 - 3 + 7) * Rnd())) + 5 & CInt(Math.Floor((111 - 56 + 2) * Rnd())) + 33
            generationAttemptLimit += 1
            If generationAttemptLimit >= 10000 Then
                MsgBox("keygen limit reached for GenerateTokenForServer")
                Return Nothing
            End If
        Loop
        ReDim Preserve ServerTokenList(ServerTokenList.Length)
        ServerTokenList(ServerTokenList.Length - 1) = newgen
        Return newgen
    End Function
    Private Function GenerateTokenForClient()
        Dim newgen As String = CInt(Math.Floor((65000 - 90 + 1) * Rnd())) + 90 & CInt(Math.Floor((11000 - 7 + 1) * Rnd())) + 7 & CInt(Math.Floor((102 - 34 + 1) * Rnd())) + 34
        Dim generationAttemptLimit As Integer = 0
        Do Until ClientTokenList.Contains(newgen) = False
            newgen = CInt(Math.Floor((65000 - 90 + 1) * Rnd())) + 90 & CInt(Math.Floor((11000 - 7 + 1) * Rnd())) + 7 & CInt(Math.Floor((102 - 34 + 1) * Rnd())) + 34
            generationAttemptLimit += 1
            If generationAttemptLimit >= 10000 Then
                MsgBox("keygen limit reached for GenerateTokenForClient")
                Return Nothing
            End If
        Loop
        ReDim Preserve ClientTokenList(ClientTokenList.Length)
        ClientTokenList(ClientTokenList.Length - 1) = newgen
        Return newgen
    End Function
    Private Function GetClientUserName(ByVal _clientToken As String)
        If _clientToken = "The Server Operator" Then
            Return " >> The Server Operator << "
        End If
        Dim claimedtoken As String = _clientToken
        Dim i As Integer = 0
        Do Until i >= ClientTokenList.Length
            If ClientTokenList(i) = claimedtoken Then
                Return ClientUserList(i)
            End If
            i += 1
        Loop
        Return Nothing
